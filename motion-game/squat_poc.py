...
def get_default_camera_index(cameras: list[tuple[int, str]]) -> int:
    """맥북 내장 카메라 인덱스 반환. MacBook/FaceTime/내장 키워드로 탐색."""
    ...
    def run(self):
        ...
        with mp_vision.PoseLandmarker.create_from_options(options) as landmarker:
            while self._running and cap.isOpened():
                ...
                # BGR → RGB 변환 후 MediaPipe Tasks API 처리
                ...
                if result.pose_landmarks:
                    ...
                    # 스쿼트: 오른쪽 엉덩이-무릎-발목
                    hip   = [lm[_RIGHT_HIP].x,   lm[_RIGHT_HIP].y]
                    knee  = [lm[_RIGHT_KNEE].x,  lm[_RIGHT_KNEE].y]
                    ankle = [lm[_RIGHT_ANKLE].x, lm[_RIGHT_ANKLE].y]
